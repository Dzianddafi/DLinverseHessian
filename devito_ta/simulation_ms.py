...
def fm_single_shot(model, geom, save=False, dt=4.0):
    ...
    Uses Devito's `AcousticWaveSolver` to generate synthetic receiver data for
    one acquisition geometry. The receiver data is resampled to the requested
    output sampling `dt`.

    Args:
        model (Model): Devito model containing the velocity field (`model.vp`)
            and numerical grid.
        ...
        save (bool, optional): Whether to save the forward wavefield during modeling.
            Defaults to False.
        dt (float, optional): Output time sampling (seconds) used when resampling
            receiver data. Defaults to 4.0.
...
def fm_multi_shots(model, geometry, n_workers, client, save=False, dt=4.0):
    ...
    This function submits `fm_single_shot` tasks in batches (up to `n_workers`
    tasks per batch), waits for completion, and gathers all shot results.
    ...
    Args:
        ...
        client: Dask client used for `submit`, `gather`, and scheduling.
        save (bool, optional): Whether to save wavefields during each forward run.
            Defaults to False.
        dt (float, optional): Output time sampling (seconds) used when resampling
            receiver data. Defaults to 4.0.

    Returns:
        list: List of per-shot results, where each element is
...
def compute_residual(residual, d_obs, d_syn, geom):
    """Compute and store the data residual (synthetic - observed).

    The residual is written in-place into `residual.data[:]`.

    Notes:
        - Current implementation assumes `d_obs` and `d_syn` already share the
          same sampling and compatible shapes.
        - A resampling/cropping version is left commented out.
    ...
    Args:
        ...
        geom (AcquisitionGeometry): Acquisition geometry (currently unused in the
            active residual formula).
...
def grad_x_shot(model_true, model_init, geom, d_obs, freq, nfilt):
    """Compute objective value and gradient contribution for one shot (filtered).

    This function computes one-shot gradient via the adjoint-state method:
      1) Forward model synthetic data using `model_init.vp`
      2) Filter synthetic data using a Butterworth lowpass filter
      3) Compute residual (synthetic - observed)
      4) Compute the L2 objective and gradient using `solver.gradient`
      5) Crop out absorbing boundary layers (nbl)

    Args:
        model_true (Model): Reference model providing grid/time settings.
        model_init (Model): Current model used to generate synthetic data.
        geom (AcquisitionGeometry): Shot geometry.
        d_obs (Receiver): Observed data for the shot.
        freq (float): Cutoff frequency (Hz) for lowpass filtering.
        nfilt (int): Filter order.

    Returns:
        tuple:
            - obj (float): Shot objective value, `0.5 * ||residual||^2`.
            - grad_i_crop (numpy.ndarray): Cropped gradient array with nbl removed.
    """
...
def normalize(data, vp):
    ...
    Args:
        ...
        vp (numpy.ndarray | float): Velocity model (or scalar). Must be broadcastable
            to `data`.
...
def grad_multi_shots(model_true, model_init, geoms, n_workers, client, d_true, freq, nfilt):
    """Compute total objective and gradient across shots in parallel (filtered).

    This function parallelizes `grad_x_shot` across a list of shot geometries
    using a Dask client. It accumulates objective values and gradients across
    all shots, applies a water-layer mute, negates the gradient (optimizer
    convention), and normalizes by `vp**3`.
