...
class AttentionGate(nn.Module):
    ...
    def __init__(self, in_channels_g, in_channels_x):
        super().__init__()
        self.conv_x = nn.Conv2d(in_channels_x, in_channels_x, kernel_size=1, stride=2, bias=False)
        self.conv_g = nn.Conv2d(in_channels_g, in_channels_x, kernel_size=1, stride=1, bias=False)
        self.relu = nn.ReLU(inplace=True)
        self.conv = nn.Conv2d(in_channels_x, 1, padding='same', kernel_size=1, bias=True)
    ...
    def forward(self, g, x):
        ...
        if x_skip.shape[2:] != x1.shape[2:]:
            x1 = F.interpolate(x1, size=x_skip.shape[2:], mode='bilinear', align_corners=True)
